...
export default () => {
  ...
  const [rows, setRows] = useState<App.Model.Product[]>([])
  const [loading, setLoading] = useState(false)
  const [rowCount, setRowCount] = useState(0)

  const columns: GridColDef[] = [
    { field: 'name', headerName: 'KODE PRODUK', flex: 1, sortable: false },
  ...
  const onFetch = async (model?: GridPaginationModel) => {
    try {
      ...
      const { data, meta } = await api.product.index(params)
