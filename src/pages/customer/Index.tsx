...
export default () => {
  ...
  const [rows, setRows] = useState<App.Model.Customer[]>([])
  const [loading, setLoading] = useState(false)
  const [rowCount, setRowCount] = useState(0)

  const columns: GridColDef[] = [
    { field: 'name', headerName: 'KODE PELANGGAN', flex: 1, sortable: false },
    { field: 'displayName', headerName: 'NAMA PELANGGAN', flex: 1, sortable: false },
    { field: 'location', headerName: 'DOMISILI', flex: 1, sortable: false, },
    {
      field: 'gender',
  ...
  const onFetch = async (model?: GridPaginationModel) => {
    try {
      ...
      const { data, meta } = await api.customer.index(params)
